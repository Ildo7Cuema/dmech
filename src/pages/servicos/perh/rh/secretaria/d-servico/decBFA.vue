<template>
  <div>
    ...
    <div style="display: none">
      <div id="elemento-para-pdf" class="StyleFontDocument c">
        ...
        <div class="row" style="font-size: 14.2px">
          <div class="col-6 text-center">
            <br />
            <b>Visto</b>
            <br />
            <span v-if="addInfo.genero == 'Masculino'"
              >O Director Provincial</span
            >
            <span v-else>A Directora Provincial</span><br /><br />
            <q-separator size="md" color="black" />
            {{ addInfo.nome_responsavel_provincial }}
          </div>
          <div class="col-6"></div>
          <div class="col-12 text-center">
            <br />
            ...
            <b style="text-transform: uppercase"
              >Declaração de Serviço para {{ addInfo.efeito_da_declaracao }}</b
            >
          </div>

          <div class="col-12 alignTextJustify" style="line-height: 1.5">
            <br />
            <span class="tab">
              Para os devidos efeitos de obtenção de
              {{ addInfo.efeito_da_declaracao }}, ao abrigo do PROTOCOLO
            </span>
            ...
            Instituição, colocado {{ artigoQantecedEscola }}
            {{ dados[0].escolas.nome }}, vinculado ao contrato de trabalho por
            tempo indeterminado, desde {{ dataInicioFuncao }}, com a função de
            <span v-if="dados[0].status_nomeacao == 'Sim'">{{
              dados[0].nome_cargo
            }}</span
            ><span v-else>{{ dados[0].categorias.categoria }}</span
            >, auferindo um vencimento líquido mensal
            <b>{{ formatCurrency(dados[0].salario_liquido) }}</b> ({{
              dados[0].salario_liquido_extenso
            }}).
            <br />
            <br />
            <span class="tab"
              >Para a garantia do cumprimento do contrato mútuo, celebrado com o
              Banco de Fomento de
            </span>
            Angola e a pedido do citado trabalhador, comprometemo-nos a
            consignar os seus salários, na conta nº
            <b>{{ dados[0].num_conta_bancaria }}</b> junto do Banco de Fomento
            de Angola até a liquidação total do Crédito.
            <br />
            <br />
            <span class="tab">Esta Direcção assume a responsabilidade de:</span>
            <br />
            <span class="tab"
              >- Comunicar ao Banco de Fomento de Angola, toda e qualquer
              alteração que se verifique no
            </span>
            Vínculo Jurídico-laboral entre o trabalhador e o Ministério da
            Educação.
            <br />
            <br />
            <span class="tab"
              >- Não permitir a mudança de Banco para pagamento do salário do
              trabalhador, sem antes
            </span>
            confirmar junto do BFA que se encontram extintas todas e quaisquer
            eventuais responsabilidades de Crédito que o mesmo tenha contraído.
            <br />
            <br />

            <span class="tab"
              >O pedido de crédito que esta declaração suporta, deverá dar
              entrada no Banco até 90 dias</span
            >
            após a emissão da mesma.
          </div>
          ...
          <div class="col-6 text-center">
            <span style="line-height: 2">
              <span v-if="addInfo.chefe_departamento_genero == 'Masculino'"
                >O Chefe do Departamento</span
              >
              <span v-else>A Chefe do Departamento</span>
              <br />
              {{ addInfo.chefe_departamento_nome }}
            </span>
          </div>
        </div>
      </div>
